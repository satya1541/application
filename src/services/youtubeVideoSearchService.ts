...
const recommendationsCache = new Map<string, { timestamp: number; videos: YouTubeVideoSearchResult[] }>();
const inFlightRecommendationsPromises = new Map<string, Promise<YouTubeVideoSearchResult[]>>();

/**
 * Fetches real-time YouTube "Up Next" / algorithmically recommended videos for any videoId
 * using YouTube InnerTube's native v1/next endpoint (identical to official YouTube recommendations).
 */
export async function fetchYouTubeNextRecommendations(
  videoId: string,
  limit: number = 20
): Promise<YouTubeVideoSearchResult[]> {
  const cleanId = videoId.replace(/^yt_/, '').trim();
  ...
  const cached = recommendationsCache.get(cleanId);
  if (cached && Date.now() - cached.timestamp < 30 * 60 * 1000) {
    return cached.videos.slice(0, limit);
  }

  // 2. Return in-flight promise if already fetching
  const existing = inFlightRecommendationsPromises.get(cleanId);
  if (existing) {
    return existing;
  }

  const fetchPromise = (async (): Promise<YouTubeVideoSearchResult[]> => {
    try {
      const visitorData = await getYouTubeVisitorData();

      // Primary client: MWEB (fast, lightweight, returns mobile watch-next feed)
      const payload = {
        context: {
          client: {
            clientName: 'MWEB',
      ...
      // 1. Parse MWEB singleColumnWatchNextResults
      const singleCol = data?.contents?.singleColumnWatchNextResults?.results?.results?.contents || [];
      for (const col of singleCol) {
        const items = col?.itemSectionRenderer?.contents || [];
        for (const it of items) {
          if (it.videoWithContextRenderer) {
            const v = it.videoWithContextRenderer;
            const targetId = v.videoId;
            if (!targetId || seenIds.has(targetId)) continue;
            seenIds.add(targetId);

            const title =
              v.headline?.runs?.map((r: any) => r.text).join('') ||
            ...
            const thumbnail =
              ...
              `https://i.ytimg.com/vi/${targetId}/hqdefault.jpg`;
            ...
            results.push({
              ...
              durationSeconds: isLive ? 0 : parseDurationSeconds(duration),
              viewCount,
              publishedTime: '',
              thumbnail,
              isLive,
            });
          } else if (it.compactVideoRenderer) {
            const v = it.compactVideoRenderer;
            const targetId = v.videoId;
            if (!targetId || seenIds.has(targetId)) continue;
            seenIds.add(targetId);
      ...
      // 2. Fallback: Parse WEB twoColumnWatchNextResults if MWEB returned 0 items
      if (results.length === 0) {
        const twoCol = data?.contents?.twoColumnWatchNextResults?.secondaryResults?.secondaryResults?.results || [];
        for (const it of twoCol) {
          if (it.compactVideoRenderer) {
            ...
            const targetId = l.contentId;
            if (!targetId || seenIds.has(targetId)) continue;
            seenIds.add(targetId);

            const title =
              l.metadata?.lockupMetadataViewModel?.title?.content || 'YouTube Video';
            const rows =
              l.metadata?.lockupMetadataViewModel?.metadata?.contentMetadataViewModel
                ?.metadataRows || [];
            const author = rows[0]?.metadataParts?.[0]?.text?.content || 'YouTube Creator';
            const viewCount = rows[1]?.metadataParts?.[0]?.text?.content
              ? `${rows[1].metadataParts[0].text.content} views`
              : '';
            ...
            const duration =
              l.contentImage?.thumbnailViewModel?.overlays?.[0]?.thumbnailBottomOverlayViewModel
      ...
      if (results.length > 0) {
        recommendationsCache.set(cleanId, {
          timestamp: Date.now(),
          videos: results,
        });
      }
      ...
      if (err?.name !== 'AbortError') {
        console.warn('fetchYouTubeNextRecommendations error:', err);
      }
      return [];
    } finally {
      inFlightRecommendationsPromises.delete(cleanId);
    }
  })();

  inFlightRecommendationsPromises.set(cleanId, fetchPromise);
  return fetchPromise;
}
