...
const STORAGE_KEY = '@deluxe_dynamic_yt_live_playlists_hd_v4';
 ...
 * and dynamically refreshes directly from YouTube InnerTube.
 */
export async function fetchDynamicYouTubePlaylists(
  ...
  // 3. Fetch fresh live channel playlists dynamically from YouTube
  try {
...
/**
 * Dynamically fetches live official YouTube playlists directly from YouTube's InnerTube browse API.
 * Queries both Indian ('IN') and Global ('US') regions in parallel to capture ALL categories:
 * Bollywood & Hindi, Punjabi & Regional, Charts, Trending, Moods, Retro, Party, Hip-Hop, and Global Pop.
 * 100% dynamic live data from YouTube with zero hardcoded playlist dependencies.
 */
async function fetchFromLiveChannel(): Promise<YouTubePlaylistItem[]> {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 9000);

  async function fetchRegion(gl: string): Promise<YouTubePlaylistItem[]> {
    try {
      const res = await fetch('https://www.youtube.com/youtubei/v1/browse?prettyPrint=false', {
        method: 'POST',
        signal: controller.signal,
        ...
        body: JSON.stringify({
          ...
          params: 'EghwbGF5bGlzdHPyBgUKA5oBAA%3D%3D',
        }),
      });
        ...
        if (node.lockupViewModel) {
          ...
          if (id && rawTitle && (id.startsWith('RDCLAK') || id.startsWith('PL') || id.startsWith('OLAK'))) {
            if (!seenIds.has(id)) {
              ...
              const sources =
                lvm.contentImage?.collectionThumbnailViewModel?.primaryThumbnail?.thumbnailViewModel?.image?.sources || [];
              let thumb = '';
              if (sources.length > 0) {
                const hdSource =
                  sources.find((s: { width?: number }) => s.width && s.width >= 500 && s.width <= 900) ||
  ...
  try {
    const [inList, usList] = await Promise.all([fetchRegion('IN'), fetchRegion('US')]);
    const seen = new Set<string>();
    const combined: YouTubePlaylistItem[] = [];

    // Prioritize Indian regional & Bollywood playlists first, then Global
